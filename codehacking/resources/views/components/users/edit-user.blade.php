@isset($user)
    <form action="{{ route('users.update', $user->id) }}" method="POST" enctype=multipart/form-data>
        @csrf @method('PATCH')
        <fieldset @disabled($user->deleted_at)>

        <input type="hidden" value="{{ $user->id }}" name="id">

        <div style="text-align:center">
            <img style="border-radius:100% ; max-height:10rem; " src="{{ $user->getProfileImage($user->image) }}"
                id="profilePreview">
        </div>
        ...
        <div class="form-group">
            ...
            <select name="gender" id="gender" value="{{ old('gender', $user->gender) }}"
